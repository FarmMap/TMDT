...
const PayProductCart = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const productIds = orderItems.map((item: { id: number }) => item.id);
        ...
        setProducts(responses.map(response => response.data) as SetStateAction<ProductType[]>);
  ...
  useEffect(() => {
   if (orderItems.length > 0) {
      const quantities = orderItems.map((item: { quantity: number }) => item.quantity);
      setQuantityState(quantities);
    }
  }, [ orderItems]);
  ...
  const handleSubmitOrder = () => {
    ...
    newOrder.orderDetails = products.map((product, index) => ({
      productId: product.id,
      quantity: quantityState[index],
      note: "",
    }));
  ...
  // thanh toan
  const totalPrice = orderItems.length > 0 
  && orderItems.reduce((total: number, item: ProductType, index: number) => {
      const quantity = quantityState[index] || 1; // Lấy quantity từ quantityState hoặc mặc định là 1
      const product = products[index];
      const salePrice = product && product.productPrice 
        ? (product.productPrice.salePrice || product.productPrice.retailPrice || 0)
        : 0;
        ...
        <Grid className={cx("wapper")}>
          ...
          <Grid item lg={8} className={cx("part-one")}>
            ...
            {products && products.map((product:ProductType, index) => (
              <Grid key={index} className={cx("product-order")}>
                ...
                <Grid className={cx("item")}>
                  ...
                  <Grid className={cx("title")}>
                    ...
                    <Grid>
                      <p style={{ color: "#000" }}>x{quantityState[index]}</p>
                    </Grid>
                  </Grid>
                </Grid>
              </Grid>
